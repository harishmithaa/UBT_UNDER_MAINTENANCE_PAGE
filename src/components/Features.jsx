  ...
  return (
    <section id="features-section" className="w-full bg-[#FFFFFF] pt-2 pb-10 px-4 md:px-8 border-t border-slate-100">
      <div className="max-w-7xl mx-auto flex flex-col gap-6">
        ...
        {/* Features 6-Column Grid with responsive layout */}
        <motion.div
        ...
        >
          {features.map((feature, idx) => (
            <motion.div
              ...
              className="card-premium rounded-xl py-4 px-4 bg-white hover:-translate-y-1 transition-all duration-300 flex flex-row sm:flex-col items-center sm:items-center text-left sm:text-center gap-4 group border border-slate-200/50 shadow-sm hover:shadow-md"
            >
              {/* Solid Green circular background with white icon */}
              <div className="h-11 w-11 rounded-full bg-primary flex items-center justify-center shrink-0 sm:mb-2 transition-transform duration-300 group-hover:scale-105 shadow-md shadow-emerald-950/5">
                {feature.icon}
              </div>
              <div className="flex flex-col gap-1 flex-grow justify-start">
                <h3 className="font-extrabold text-secondary text-sm sm:text-sm leading-tight transition-colors duration-300 group-hover:text-primary">
                  {feature.title}
                </h3>
                <p className="text-slate-500 text-sm sm:text-xs leading-relaxed mt-0.5">
                  {feature.desc}
                </p>
              </div>
            </motion.div>
